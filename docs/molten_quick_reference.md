...
- `:NotebookSetup` - Interactive setup for any Python project type
- `:WebSetup` - Quick setup for web development (FastAPI/Flask/Django)
- `:NotebookNew [filename] [language]` - Create new Jupyter notebook
- `:NotebookCheck` - Run diagnostics to verify installation
- `:NotebookInstall <packages>` - Install ANY Python package from PyPI
- `:NotebookPackages` - Show common packages with categories
- `:NotebookEnv` - Show Python environment status

## Getting Started

### For Data Science Projects
1. Run `:NotebookSetup` and choose "Data Science / Notebooks"
2. Create notebook: `:NotebookNew [filename]`
3. Initialize kernel: `<leader>ji`

### For Web Development
1. Run `:WebSetup` (or `:NotebookSetup` → choose "Web Development")
...
nvim
# :NotebookSetup → Choose "Data Science"
# Restart nvim
# :NotebookNew analysis
...
## Project Types in NotebookSetup
...
## Supported Languages
- Python (default)
- Julia (`:NotebookNew notebook julia`)
- R (`:NotebookNew notebook r`)
